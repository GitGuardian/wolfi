FROM reg.echohq.com/base-fips-python:builder AS builder

RUN add-pkg -r \
      libstdc++6 python-3.13 ca-certificates \
      bash busybox jq netcat-openbsd openssl wget \
...
# add-pkg does not run busybox's applet-link step, so the runtime would ship the
# busybox binary with none of the ls/cat/grep/sed/id/tar commands the shell
# variant exists to provide. Wolfi's busybox apk installs these links itself.
# Existing real binaries are never clobbered.
RUN for a in $(chroot /os /usr/bin/busybox --list); do \
      [ -e "/os/usr/bin/$a" ] || ln -s busybox "/os/usr/bin/$a"; \
    done
